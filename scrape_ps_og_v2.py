...
1. YOU log in manually (handles Cloudflare + 2FA naturally)
1. Script detects when login is complete and starts scraping automatically
1. Session is saved to disk – next run skips login entirely

## What it does

1. Reads gmatclub_tags.xlsx (built in Step 1) to get the 14 PS Source:OG tag IDs
1. For each tag ID, paginates through ALL search result pages (start=0,50,100…)
1. From every question card captures:
   Title, Link, Category, Tags (labels), Tag IDs
1. Deduplicates globally – a question under multiple OG tags is saved only once
1. Saves a master Excel:
